...
| Action | Description |
|--------|-------------|
| `REGISTERED` | Device registration initiated |
...
4. Send OTP to brand email
5. Store OTP with 10-minute expiry in cache (Redis/memory)
6. Return success with expiry timestamp
...
```json
{
  ...
  "data": {
    ...
    "message": "Device registered successfully. Waiting for manager approval."
  }
}
```
...
5. If OTP valid:
   ...
   - Create registered_devices record with approval_status='PENDING', is_active=false
   - Create audit log entry (REGISTERED action)
   - Clear OTP from cache
6. Return response with device ID and pending status (no device token yet)
...
4. If approve=true:
   ...
   - Create audit log entry (APPROVED action)
   - Return device token
5. If approve=false:
   - Set approval_status to REJECTED
   - Set is_active to false
   - Create audit log entry (REJECTED action)
...
4. Create audit log entry (REVOKED action) with reason
...
4. Set approval_status to PENDING (requires re-approval at new store)
5. Set is_active to false
6. Create audit log entry (TRANSFERRED action) with transfer details
7. All users must re-register the device at the new store
...
```json
{
  ...
  "message": "Device is waiting for manager approval."
}
```
...
7. If device pending approval:
   - Return error: `DEVICE_PENDING_APPROVAL`
8. If device rejected:
   - Return error: `DEVICE_REJECTED`
...
**Updated Response (Error - Device Pending Approval):**
```json
{
  "success": false,
  "error": "DEVICE_PENDING_APPROVAL",
  "message": "Your device is waiting for manager approval. Please contact your manager."
}
```

**Updated Response (Error - Device Rejected):**
```json
{
  "success": false,
  "error": "DEVICE_REJECTED",
  "message": "Your device registration was rejected. Please contact your manager."
}
```
  ...
  try {
    ...
    const device = await db.query.registeredDevices.findFirst({
      where: and(
        ...
        eq(registeredDevices.approvalStatus, 'APPROVED'),
        gt(registeredDevices.expiresAt, new Date())
      )
    });
    
    if (!device) {
      const pendingDevice = await db.query.registeredDevices.findFirst({
        where: and(
          eq(registeredDevices.id, tokenData.deviceId),
      ...
      const rejectedDevice = await db.query.registeredDevices.findFirst({
        where: and(
          eq(registeredDevices.id, tokenData.deviceId),
          eq(registeredDevices.approvalStatus, 'REJECTED')
        )
      });
...
**Step 3: Registration Complete (Pending Approval)**
```
┌─────────────────────────────────────────────────────────┐
│  Registration Submitted                                 │
│  ────────────────────────                               │
│                                                         │
│  ⏳ Your device registration has been submitted         │
│                                                         │
...
async function initializeApp() {
  ...
  // 2. Check device deviceToken = local registration
  constStorage.getItem('pos-device-token');
  ...
  try {
    ...
    if (status.isPendingApproval) {
      navigate('/device/register?status=pending');
      return;
    }
...
| DEVICE_PENDING_APPROVAL | 403 | Device awaiting approval | Device waiting for manager approval. |
| DEVICE_REJECTED | 403 | Device rejected by manager | Device registration was rejected. |
...
| Device registration flow | Init → OTP → Verify → Device created as PENDING |
| Manager approval | POST /devices/approve, device becomes ACTIVE |
| Manager rejection | POST /devices/approve (approve=false), device becomes REJECTED |
| User login with device | Login with approved device token |
| User login pending | Login with pending device → error |
| User login mismatch | Login with wrong store → error |
| Device revocation | Revoke device, user gets logged out |
| Device transfer | Transfer device, requires re-approval at new store |
...
| Test | Description |
|------|-------------|
| Full device registration | Complete flow from store code to pending approval |
| Manager approval | Manager approves pending device, user logs in |
| Manager rejection | Manager rejects device, user sees rejection message |
| Device transfer | Transfer device between stores, verify re-approval required |
| Device expiration | Device expires, user forced to re-register |
...
### 8.5 Approval Workflow Security

- Only managers can approve devices for their store
- Device is not active until approved
- Rejected devices cannot be used
- All approval/rejection actions logged with approver ID
...
| 1.1 | 2025-01-11 | - | Restored approval flow for better security control |
