...
4. Device binds to store → User selection screen appears
5. Manager selects their profile → Enters PIN → Authenticated

**Scenario 2: Daily Login (Cashier)**

1. Cashier opens POS app
2. Device already bound → User selection screen shows
3. Cashier taps their profile → Enters 6-digit PIN
4. Authenticated → POS dashboard loads

**Scenario 3: Device Revocation (Admin)**

1. Admin opens Admin Portal → Devices menu
...
| TC-10 | Cashier fails PIN 5 times → locked for 15 min |
| TC-11 | User switches to different user |
| TC-12 | Admin revokes device → users logged out |
...
│   ├── auth.ts                   # Device login, PIN login
│   └── devices.ts                # Device-authenticated endpoints
├── utils/
│   ├── device.ts                 # Code generation, QR generation
│   └── auth.ts                   # PIN hash/verify
├── db/
│   └── schema.ts                 # Updated with device_bindings, user_sessions
├── middleware/
│   └── deviceAuth.ts             # Device authentication middleware
└── jobs/
    ├── sessionCleanup.ts         # Clean up inactive sessions
    └── deviceExpiration.ts       # Expire unused binding codes

apps/api/drizzle/
└── 000X_device_binding_auth.sql   # Migration file
...
│       └── PinEntryScreen.tsx
├── pages/
│   └── Login.tsx                  # Updated to check device binding status
└── utils/
...
│   └── Users.tsx                  # Updated with PIN management
└── components/
    └── devices/
        ├── DeviceList.tsx
...
packages/shared/src/
└── types.ts                       # Updated with DeviceBinding, UserSession types

packages/api/src/
└── apiClient.ts                   # Updated with device/auth methods
```
