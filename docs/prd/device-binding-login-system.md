...
┌─────────────────────────────────────────────────────────────────────────────────┐
│                                                                                 │
│  PHASE 1: Device Setup (One-time by Admin via Admin Portal)                    │
│  ┌─────────────────────────────────────────────────────────────────────────┐   │
│  │  Admin Portal                                                            │   │
│  │  ├── Select Store                                                       │   │
│  │  ├── Create Device → System auto-generates:                             │   │
│  │  │   ├── Device Name: "POS-[RandomID]" (e.g., POS-A7X9K2)             │   │
│  │  │   ├── 6-char Code: "X7K9P2"                                        │   │
│  │  │   └── QR Code (contains code + store info)                         │   │
│  │  └── Store Manager scans QR or enters code on POS device               │   │
│  └─────────────────────────────────────────────────────────────────────────┘   │
│                                      │                                            │
│                                      ▼                                            │
│  ┌─────────────────────────────────────────────────────────────────────────┐   │
│  │  PHASE 2: First-Time Device Login (Store Manager)                      │   │
│  │  POS Device Login Screen                                                │   │
│  │  ├── Option A: Scan QR Code                                             │   │
│  │  ├── Option B: Enter 6-char code manually                               │   │
│  │  └── System validates → Device bound to store (1:1 relationship)       │   │
│  └─────────────────────────────────────────────────────────────────────────┘   │
│                                      │                                            │
│                                      ▼                                            │
│  ┌─────────────────────────────────────────────────────────────────────────┐   │
│  │  PHASE 3: Daily POS Login (Cashier / Store Manager)                     │   │
│  │  POS Login Screen (after device is bound)                                │   │
│  │  ├── Shows: Store Name, Device Name, List of Users                     │   │
│  │  ├── User taps their profile                                            │   │
│  │  ├── PIN entry screen appears                                          │   │
│  │  ├── User enters 6-digit PIN                                            │   │
│  │  └── Authenticated! → Full POS access                                   │   │
│  └─────────────────────────────────────────────────────────────────────────┘   │
│                                      │                                            │
│                                      ▼                                            │
│  ┌─────────────────────────────────────────────────────────────────────────┐   │
│  │  PHASE 4: Admin Revocation (Anytime)                                    │   │
│  │  Admin Portal                                                           │   │
│  │  ├── Admin clicks "Revoke" on device                                    │   │
│  │  └── POS device automatically logs out all users                       │   │
│  └─────────────────────────────────────────────────────────────────────────┘   │
│                                                                                 │
└─────────────────────────────────────────────────────────────────────────────────┘
```
...
│                                                                                 │
│    Store:                                                                   │
│    - One Store Manager (required)                                             │
│    - Multiple Cashiers (no limit)                                            │
│                                                                                 │
│    Device Binding:                                                            │
│    - Bound to ONE store                                                      │
│    - Has ONE active user session at a time                                   │
│                                                                                 │
│    User:                                                                      │
│    - Belongs to ONE store                                                    │
│    - Has ONE role (Manager or Cashier)                                       │
│    - Has ONE PIN for authentication                                          │
│                                                                                 │
└─────────────────────────────────────────────────────────────────────────────────┘
```
...
|---------|---------------|---------|
| Create new device | Own store only | ❌ No |
| Bind device to store | Own store only | ❌ No |
| Add/remove users | Own store only | ❌ No |
| Reset user PIN | Own store only | ❌ No |
| View all transactions | All | Own only |
| End of Day | ✅ Yes | ❌ No |
| Refund transactions | ✅ Yes | ⚠️ Limited |
| Discount override | ✅ Yes | ⚠️ Limited |
...
| device_id | uuid | FK → devices.id, NULLABLE | Linked device (NULL until claimed) |
...
| revoked_by | uuid | FK → users.id, NULLABLE | Who revoked |
...
| device_id | uuid | FK → devices.id, NOT NULL | Bound device |
| store_id | uuid | FK → stores.id, NOT NULL | Store (redundant for query speed) |
...
| Column | Type | Constraints | Description |
|--------|------|--------------|-------------|
| binding_id | uuid | FK → device_bindings.id, NULLABLE | Current binding |
...
**Location:** Admin Sidebar → "Devices"
...
**Access:** Devices Menu → "Create Device" button OR Store → Devices Tab → "Create Device"
...
| **Revoke** | Open confirmation modal → Force logout all users |
...
**Manager Constraint:**
- If role = Manager and store already has a manager → Show error: "Store already has a Manager. Please assign Cashier role or remove existing Manager first."
...
│                                                                 │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │  [ 📷 Scan QR Code ]     [ ⌨️ Enter Code Manually ]     │   │
│  └─────────────────────────────────────────────────────────┘   │
│                                                                 │
│  ──────────────────────────────────────────────────────────    │
│                                                                 │
│  QR SCAN MODE (default if camera available):                   │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │                                                         │   │
│  │              [ Camera Viewfinder ]                      │   │
│  │                                                         │   │
...
**Interactions:**
- Tap/click user card → Navigate to PIN entry screen
- "Switch Device" button → Navigate to device login screen (for different store)
...
**Location:** POS Header → User avatar → "Switch User"
...
**Rules:**
- 30 minutes of inactivity → User session expires
- "Inactivity" = no API calls from POS device
- On timeout → Show PIN entry screen (device remains bound)
...
| Method | Endpoint | Description |
|--------|----------|-------------|
| POST | `/auth/device-login` | Login with binding code → returns device + users |
...
2. Navigates to Devices Menu or Store → Devices Tab
...
7. Manager taps "Continue" → User selection screen
8. Manager selects their profile
9. Enters PIN → Authenticated
...
Admin Portal                    POS Application
     ...
     ├── Create Device ──────────────┤
     │                               │
     ├── View Devices ───────────────┼── User Selection
     │                               │
     ├── Revoke Device ──────────────┼── Session Validation
     │                               │
     └── User Management ────────────┼── PIN Authentication
                                      │
                                      └── Session Creation
```
